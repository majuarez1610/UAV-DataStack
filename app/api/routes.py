...
def system_status():
    ...
    try:
        ...
        if vs:
            video_available = bool(vs.status().get("available", False))
    except Exception:
        pass
        ...
        out.append(
            {
                ...
                "name": m.name,
                "status": m.status,
                "start_ts": m.start_ts.isoformat() if m.start_ts else None,
...
def video_status():
    try:
        ...
        vs = getattr(_ext, "video_service", None)
        if not vs:
            return jsonify({"ok": True, "video": {"available": False, "opened": False}}), 200
        st = vs.status()
